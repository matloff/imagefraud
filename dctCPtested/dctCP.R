...
Q <- 48 #63 JPEG Quality factor: found by trial and error but might be command line arg to print this from image
Nf <- 30 #110 should print row/column pairs of distances greater than Nf (adjust to print number of copied regions)
...
for (i in 1:(numFound-1)){
  for (j in 1:nrow(freqPairs)){
    ...
    imageInCopy[pairLoc1[i,1]:(pairLoc1[i,1]+boxside - 1), pairLoc1[i,2]:(pairLoc1[i,2]+boxside - 1),1:dim3] = 0
    imageInCopy[pairLoc2[i,1]:(pairLoc2[i,1]+boxside - 1), pairLoc2[i,2]:(pairLoc2[i,2]+boxside - 1),1:dim3] = 0
    }
  }
}
))

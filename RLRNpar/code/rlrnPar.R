...
print(system.time(testRLRNpar(authenticDir, tamperDir, pfeatures, par, newImagesDir)))))
  ...
  # down diagonal direction (invalid subscript error is here and other diagonal in parallel)
  end <- 0
  for (i in 1:(width-2)){
    for (j in 1:(height-2)){
      end <- 0
...
testRLRNpar <- function(authenticDir, tamperDir, pfeatures, par=4){
  ...
  allImagesArray <- cbind(allImagesArray, truthVector)
  allImagesArray <-as.data.frame(allImagesArray)
  allImagesArray$V61 <- as.factor(allImagesArray$V61)
  ...
  # print(system.time(clusterEvalQ(cls,tune <- tune.svm(V61 ~., data = train, gamma =seq(.01, 0.1, by = .01), cost = seq(0.1,1, by = 0.1)))))
  # print(system.time(clusterEvalQ(cls, mysvm <- svm(V61 ~., data = train, gamma = tune$best.parameters$gamma, cost = tune$best.parameters$cost))))
  # print(system.time(predictC <- clusterEvalQ(cls, svmpredict <- predict(mysvm, test, type = "response"))))
  # predictsvmC <-Reduce('+', predictC) / 2 # <- needs to be changed
  ...
  # 10-fold cross-validation is employed in classification
  # first paper: repeated 30 times for each parameter group (C,γ)
  tune <- tune.svm(V61 ~., data = train, gamma =seq(.01, 0.1, by = .01), cost = seq(0.1,1, by = 0.1))
  print(tune$best.parameters)

  # first paper: the performance is measured by the average classification accuracy across the 30 runs
  mysvm <- svm(V61 ~., data = train, gamma = tune$best.parameters$gamma, cost = tune$best.parameters$cost)
  print(summary(mysvm))
  svmpredict <- predict(mysvm, test, type = "response")
  print(table(pred=svmpredict, true=test$V61))
}

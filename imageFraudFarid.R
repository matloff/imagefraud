...
for (i in 1:(width-boxside+1)){
  ...
  test[(i+j-1),1:64] <- as.vector(round(rescale(approx[j:endh,i:endw], to = c(0, 256))))
  # save location
  test[(i+j-1),65:66] <- c(i,j)
  }}

))
# sort by all columns
print(system.time(test <- test[do.call(order, lapply(1:64, function(i) test[,i])),]))
